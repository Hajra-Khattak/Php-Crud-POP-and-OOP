...
if(isset($_POST["submit"]) || !empty($_POST["submit"]) ){
    ...
    $fileData = File_upload("image", $validImageExtension, "assets/images/");
    ...
    if($fileData == 1){
        ...
        $status['errors']++;
        array_push($status["msg"], "SIZE IS NOT VALID");
    }else{
        $encode = json_encode($fileData);
    }
...
*/
    ...
    // } else {
    //     $status["errors"]++;
    //     array_push($status["msg"], "Image upload failed.");
    // }
        ...
        $insert = "INSERT INTO `user`(`name`,`email`,`image`,`password`) VALUES('{$name}','{$email}','{$fileData}','{$encrypt}')";
